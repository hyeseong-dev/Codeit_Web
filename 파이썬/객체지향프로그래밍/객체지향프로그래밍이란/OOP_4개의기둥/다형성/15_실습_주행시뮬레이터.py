# -*- encoding: utf8 -*-
'''
이 세 가지 조건을 만족해야됩니다. 주행 시뮬레이터는 템플릿에 DrivingSimulator라는 이름으로 미리 적어두었습니다. DrivingSimulator클래스가 완성되었을때
아래 코드의 실행 결과와 동일한 결과가 나와야합니다. DrivingSimulator클래스의 각 메소드를 완성해보세요.
'''
...
class DrivingSimulator:
    ...
    def add_vehicle(self, new_vehicle):
        ...
        if isinstance(new_vehicle, Vehicle): 
            ...
            return print(f'{new_vehicle}은 교통수단이 아니기 때문에 추가할 수 없습니다.')
    ...
    def __str__(self):
        ...
        for vehicle in self.vehicles : 
            vehicle_speeds.append(f'{vehicle}\n')
        return "".join(vehicle_speeds)
